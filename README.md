...
<li>Lotus startup will be a little bit slow, let's say more than 20 seconds. Tap two or three times -enter- and -space- and don't panic, just wait. Once run, I usually leave this in background, so tapping again the FreeDOS icon will bring it back immediatly when needed.
...
<li>If you get too much memory on your BOCHS machine (<b>megs:</b> option in bochsrc), Lotus startup will be even slower (!), in my setup 512 megs even prevent BOCHS starting. 256 megs should be fine.
...
Well, you have to kill it via terminal, or just reboot reMarkable, You can also ssh again to your reMarkable, kill it via a yaft terminal, or install tmux to have access a second shell if you run your Lotus via ssh. The startup options in the BOCHS command line provided, will ignore .lock files remaining when BOCHS is killed. A simple script to get the PID and kill it right away is included in the repository. Note: you can leave it running, it will not kill your device performance, plus, once you tap again on the FreeDOS icon the same istance will come back, without running a new one. I usually leave it open once rM starts, and use it throughout the day.
